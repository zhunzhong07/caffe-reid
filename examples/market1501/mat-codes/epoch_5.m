...
score_1L = importdata('../datamat/train.1L.epoch5.score.mat');
score_1  = importdata('../datamat/train.1.epoch5.score.mat');
fc7_1L = importdata('../datamat/train.1L.epoch5.fc7.mat');
fc7_1  = importdata('../datamat/train.1.epoch5.fc7.mat');
...
for label = 1:numel(unique_label)
	...
	if (numel(select_per_class{label}) > 0)
		...
		temp = temp(find(Mmx_score(temp)>0.3));
	...
	fprintf('Label %3d, generate %2d / %2d / %2d pesudo label, ok : %2d, accuracy : %.3f, score : %.3f\n', label, numel(select_per_class{label}), count_per_class(label), numel(find(label_1L==label)), numel(find(true_label==label)), mean(true_label==label), min(Smx_score(select_per_class{label})));
...
%% Normalize            : accuracy: 0.90320  || zero_acc : 31

list_file = fopen('../lists/train.epoch.6', 'w');

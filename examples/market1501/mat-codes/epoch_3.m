...
score_1L = importdata('../datamat/train.1L.epoch3.score.mat');
score_1  = importdata('../datamat/train.1.epoch3.score.mat');
fc7_1L = importdata('../datamat/train.1L.epoch3.fc7.mat');
fc7_1  = importdata('../datamat/train.1.epoch3.fc7.mat');
...
for label = 1:numel(unique_label)
	count_per_class(label) = 3*numel(find(label_1==label));
	count_per_class(label) = min(count_per_class(label), numel(find(label_1L==label)));
end
...
for label = 1:numel(unique_label)
	...
	if (numel(select_per_class{label}) > 0)
		...
		temp_score = min(Mmx_score(temp));
		temp = temp(find(Mmx_score(temp)>0.5));
	...
	fprintf('Label %3d, generate %2d / %2d pesudo label, ok : %2d, accuracy : %.3f, score : %.3f\n', label, select, numel(find(label_1L==label)), numel(find(true_label==label)), mean(true_label==label), temp_score);
...
%% Normalize            : accuracy: 0.90728  ||  un_select : 12 || zero_acc : 46

list_file = fopen('../lists/train.epoch.4', 'w');
